<main class="container">
    ...
    <form method="POST">
        <ul class="collection" style="max-height: 300px; overflow-y: scroll">
            <?php if (isset($resultado) && count($resultado) > 0): foreach ($resultado as $usuario): ?>
                    <li class="collection-item">
                        <input class="limited" type="checkbox" id="<?= $usuario->getId() ?>" value="<?= $usuario->getId() ?>" name="id[]"/>
                        <label for="<?= $usuario->getId() ?>" ><?= $usuario->getNome() ?></label>
                        <span class="secondary-content">CPF: <span class="cpf"><?= $usuario->getCpf() ?></span></span>
