<main class="container">
    ...
    <form method="POST">
        <ul class="collection" style="max-height: 300px; overflow-y: scroll">
            <?php if (isset($resultado) && count($resultado) > 0): foreach ($resultado as $setor): ?>
                    <li class="collection-item">
                        <input class="limited" type="checkbox" id="<?= $setor->getId() ?>" value="<?= $setor->getId() ?>" name="id[]"/>
                        <label for="<?= $setor->getId() ?>" ><?= $setor->getNome() ?></label>
                    </li>
                <?php endforeach;
            else:
                ?>
